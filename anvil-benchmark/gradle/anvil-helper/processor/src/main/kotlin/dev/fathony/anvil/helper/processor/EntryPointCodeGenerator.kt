...
class EntryPointCodeGenerator : CodeGenerator {
    ...
    override fun generateCode(
        ...
        return projectFiles
            ...
            .filter { it.isAnnotatedWith(DefineEntryPointFqName) }
            .onEach { clazz ->
                clazz.checkClassIsPublic {
                    "${clazz.fqName} needs to be public to become an Entry Point."
                }
                clazz.checkDefineEntryPointRequirement()
            }
            .map { clazz ->
                val entryPointClassName = clazz
                            ...
                            FunSpec.builder("bind${entryPointClassName}")
                                ...
                                .addAnnotation(
                                    AnnotationSpec.builder(ClassKey::class)
                                        .addMember("value = %T::class", clazz.asClassName())
                                        .build()
                                )
                                .addModifiers(KModifier.ABSTRACT)
                                .addParameter(
                                ...
                                .returns(
                                    EntryPoint.Factory::class.asClassName().parameterizedBy(STAR)
                                )
                                .build()
                        )
                }
